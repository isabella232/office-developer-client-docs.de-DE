---
title: Connection Properties, Methods, and Events (ADO)
TOCTitle: Properties, Methods, and Events
ms:assetid: e78329a4-0b90-9ae5-f3d7-e56815a396fd
ms:mtpsurl: https://msdn.microsoft.com/library/JJ250174(v=office.15)
ms:contentKeyID: 48548407
ms.date: 09/18/2015
mtps_version: v=office.15
ms.openlocfilehash: df0ab6516fbe2115abecf5105bde4ff0f790087b
ms.sourcegitcommit: c557bbcccf37a6011f89aae1ddd399dfe549d087
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 10/31/2018
ms.locfileid: "25873684"
---
# <a name="connection-properties-methods-and-events-ado"></a>Connection Properties, Methods, and Events (ADO)

**Betrifft**: Access 2013, Office 2013

## <a name="propertiescollections"></a>Eigenschaften/Auflistungen

- [Attributes-Eigenschaft](attributes-property-ado.md)

- [CommandTimeout-Eigenschaft](commandtimeout-property-ado.md)

- [ConnectionString-Eigenschaft](connectionstring-property-ado.md)

- [ConnectionTimeout-Eigenschaft](connectiontimeout-property-ado.md)

- [CursorLocation-Eigenschaft](cursorlocation-property-ado.md)

- [DefaultDatabase-Eigenschaft](defaultdatabase-property-ado.md)

- [Errors-Auflistung](errors-collection-ado.md)

- [IsolationLevel-Eigenschaft](isolationlevel-property-ado.md)

- [Mode-Eigenschaft](mode-property-ado.md)

- [Properties-Auflistung](properties-collection-ado.md)

- [Provider-Eigenschaft](provider-property-ado.md)

- [State-Eigenschaft](state-property-ado.md)

- [Version-Eigenschaft](version-property-ado.md)

## <a name="methods"></a>Methoden

- [Methoden "BeginTrans", "CommitTrans" und "RollbackTrans" (ADO)](begintrans-committrans-and-rollbacktrans-methods-ado.md)

- [Cancel-Methode](cancel-method-ado.md)

- [Close-Methode](close-method-ado.md)

- [Execute-Methode (ADO-Verbindung)](https://msdn.microsoft.com/library/jj249832\(v=office.15\))

- [Open-Methode (ADO-Verbindung)](open-method-ado-connection.md)

- [OpenSchema-Methode](openschema-method-ado.md)

## <a name="events"></a>Ereignisse

- [BeginTransComplete-, CommitTransComplete- und RollbackTransComplete-Ereignisse (ADO)](begintranscomplete-committranscomplete-and-rollbacktranscomplete-events-ado.md)

- [ConnectComplete- und Disconnect-Ereignisse (ADO)](connectcomplete-and-disconnect-events-ado.md)

- [ExecuteComplete-Ereignis](executecomplete-event-ado.md)

- [InfoMessage-Ereignis](infomessage-event-ado.md)

- [WillConnect-Ereignis](willconnect-event-ado.md)

- [WillExecute-Ereignis](willexecute-event-ado.md)

