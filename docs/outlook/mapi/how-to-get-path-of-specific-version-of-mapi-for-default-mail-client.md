...
---
# <a name="get-the-path-of-a-specific-version-of-mapi-for-the-default-mail-client"></a><span data-ttu-id="49aa4-103">Abrufen des Pfads einer bestimmten Version von MAPI für den Standard-Mail-client</span><span class="sxs-lookup"><span data-stu-id="49aa4-103">Get the path of a specific version of MAPI for the default mail client</span></span>

<span data-ttu-id="49aa4-104">**Betrifft**: Outlook 2013 | Outlook 2016</span><span class="sxs-lookup"><span data-stu-id="49aa4-104">**Applies to**: Outlook 2013 | Outlook 2016</span></span> 
  
<span data-ttu-id="49aa4-105">Dieses Thema enthält ein Codebeispiel in C++, das zeigt, wie Sie den Pfad einer bestimmten Version von MAPI zu erhalten, die von der standardmäßige e-Mail-Client auf einem Computer verwendet wird.</span><span class="sxs-lookup"><span data-stu-id="49aa4-105">This topic includes a code sample in C++ that shows how to obtain the path of a specific version of MAPI that is used by the default mail client on a computer.</span></span> <span data-ttu-id="49aa4-106">MAPI-Mail-Clients haben die Möglichkeit, die in der Registrierung angeben, die eine benutzerdefinierte DLL, dass die MAPI-Stub-Bibliothek laden und Versenden von MAPI sollte aufgerufen.</span><span class="sxs-lookup"><span data-stu-id="49aa4-106">MAPI mail clients have an option to specify in the registry a custom DLL that the MAPI stub library should load and dispatch MAPI calls to.</span></span> <span data-ttu-id="49aa4-107">Der Registrierungsschlüssel für diese benutzerdefinierte DLL für standardmäßige e-Mail-Client festgelegt ist **MSIComponentID**, unter dem Schlüssel **HKLM\Software\Clients\Mail** der standardmäßige e-Mail-Client.</span><span class="sxs-lookup"><span data-stu-id="49aa4-107">The registry key to set for this custom DLL for a default mail client is **MSIComponentID**, under the **HKLM\Software\Clients\Mail** key of the default mail client.</span></span> <span data-ttu-id="49aa4-108">Die [FGetComponentPath](fgetcomponentpath.md) -Funktion, durch die MAPI-Stub-Bibliothek exportiert mapistub.dll, können den Pfad zu der benutzerdefinierten Version von MAPI durch den Registrierungsschlüssel **MSIComponentID** angegebenen zurückzugeben.</span><span class="sxs-lookup"><span data-stu-id="49aa4-108">The [FGetComponentPath](fgetcomponentpath.md) function, exported by the MAPI stub library, mapistub.dll, can return the path to the custom version of MAPI specified by the **MSIComponentID** registry key.</span></span> 
  
<span data-ttu-id="49aa4-109">In diesem Codebeispiel enthält zwei Funktionen: `HrGetRegMultiSZValueA` und `GetMAPISVCPath`.</span><span class="sxs-lookup"><span data-stu-id="49aa4-109">This code sample includes two functions:  `HrGetRegMultiSZValueA` and  `GetMAPISVCPath`.</span></span> <span data-ttu-id="49aa4-110">Die `GetMAPISVCPath` -Funktion [FGetComponentPath](fgetcomponentpath.md) verwendet, um den Pfad zu der benutzerdefinierten MAPI-Version zu erhalten.</span><span class="sxs-lookup"><span data-stu-id="49aa4-110">The  `GetMAPISVCPath` function uses [FGetComponentPath](fgetcomponentpath.md) to obtain the path to the custom version of MAPI.</span></span> <span data-ttu-id="49aa4-111">Es wird davon ausgegangen, dass der standardmäßige e-Mail-Client Microsoft Office Outlook 2007 ist und den Wert **FGetComponentPath übergibt** `{FF1D0740-D227-11D1-A4B0-006008AF820E}`, mit dem Outlook 2007 festgelegt wird, wie die Komponente der MAPI-ID für den Registrierungsschlüssel **MSIComponentID** .</span><span class="sxs-lookup"><span data-stu-id="49aa4-111">It assumes that the default mail client is Microsoft Office Outlook 2007, and passes to **FGetComponentPath** the value,  `{FF1D0740-D227-11D1-A4B0-006008AF820E}`, that Outlook 2007 sets as the component ID of MAPI for the **MSIComponentID** registry key.</span></span> 
  
> [!NOTE]
> <span data-ttu-id="49aa4-112">In der Praxis sollten Sie nicht den Wert angenommen `{FF1D0740-D227-11D1-A4B0-006008AF820E}`, handelt es sich immer die ID des MAPI-Komponente und direkt an **FGetComponentPath**übergeben.</span><span class="sxs-lookup"><span data-stu-id="49aa4-112">In practice, you should not assume the value,  `{FF1D0740-D227-11D1-A4B0-006008AF820E}`, is always the component ID of MAPI and directly pass it to **FGetComponentPath**.</span></span> <span data-ttu-id="49aa4-113">Um zuverlässig herauszufinden, welche Version von Outlook MAPI auf einem Computer verwendet, müssen Sie den Wert der **MSIComponentID** aus der Registrierung lesen und an **FGetComponentPath**übergeben.</span><span class="sxs-lookup"><span data-stu-id="49aa4-113">To reliably find out which version of MAPI Outlook uses on a computer, you must read from the registry the value of **MSIComponentID** and pass it to **FGetComponentPath**.</span></span> 
  
<span data-ttu-id="49aa4-114">Die folgenden Schritte beschreiben, wie `GetMAPISVCPath` dies geschieht.</span><span class="sxs-lookup"><span data-stu-id="49aa4-114">The following steps describe how  `GetMAPISVCPath` does this.</span></span> 
  
1. <span data-ttu-id="49aa4-115">Lädt die MAPI-Stub-Bibliothek mapistub.dll, aus dem Verzeichnis des Systems.</span><span class="sxs-lookup"><span data-stu-id="49aa4-115">Loads the MAPI stub library, mapistub.dll, from the system directory.</span></span>
    
2. <span data-ttu-id="49aa4-116">Wird davon ausgegangen, dass mapistub.dll exportiert die **FGetComponentPath** -Funktion, wird versucht, die Adresse dieser Funktion von mapistub.dll abgerufen.</span><span class="sxs-lookup"><span data-stu-id="49aa4-116">Assumes that mapistub.dll exports the **FGetComponentPath** function, it tries to get the address of this function from mapistub.dll.</span></span> 
    
3. <span data-ttu-id="49aa4-117">Wenn abrufen, die Adresse aus mapistub.dll fehlschlägt, wird versucht, die Adresse aus mapi32.dll abzurufen.</span><span class="sxs-lookup"><span data-stu-id="49aa4-117">If getting the address from mapistub.dll fails, it tries to get the address from mapi32.dll.</span></span>
    
4. <span data-ttu-id="49aa4-118">Wenn die Adresse des **FGetComponentPath** erste erfolgreich ist, wird die Registrierung geöffnet und verwendet die `HrGetRegMultiSZValueA` Funktion zum Lesen der Registrierungswerte unter **HKLM\Software\Clients\Mail\Microsoft Outlook**.</span><span class="sxs-lookup"><span data-stu-id="49aa4-118">If getting the address of **FGetComponentPath** succeeds, it opens the registry and uses the  `HrGetRegMultiSZValueA` function to read the registry values under **HKLM\Software\Clients\Mail\Microsoft Outlook**.</span></span> 
    
5. <span data-ttu-id="49aa4-119">Ruft **FGetComponentPath**, Angeben des Werts `{FF1D0740-D227-11D1-A4B0-006008AF820E}`, um den Pfad zu der MAPI-Version abzurufen, die Outlook 2007 verwendet.</span><span class="sxs-lookup"><span data-stu-id="49aa4-119">Calls **FGetComponentPath**, specifying the value,  `{FF1D0740-D227-11D1-A4B0-006008AF820E}`, to obtain the path to the version of MAPI that Outlook 2007 uses.</span></span>
    
<span data-ttu-id="49aa4-120">Beachten Sie, dass zur Unterstützung der lokalisierter Kopien der MAPI für Englisch und nicht-englischen Gebietsschemas im Codebeispiel die Werte für die Unterschlüssel **MSIApplicationLCID** und **MSIOfficeLCID liest** und **FGetComponentPath**, zuerst angeben **von Anrufen MSIApplicationLCID** als *SzQualifier* , und klicken Sie dann erneut angeben **MSIOfficeLCID** als *SzQualifier* .</span><span class="sxs-lookup"><span data-stu-id="49aa4-120">Note that to support localized copies of MAPI for English and non-English locales, the code sample reads the values for the **MSIApplicationLCID** and **MSIOfficeLCID** subkeys and calls **FGetComponentPath**, first specifying **MSIApplicationLCID** as  *szQualifier*  , and then again specifying **MSIOfficeLCID** as  *szQualifier*  .</span></span> <span data-ttu-id="49aa4-121">Weitere Informationen zu Registrierungsschlüsseln für e-Mail-Clients, die nicht-englischen Sprachen unterstützen, finden Sie unter [Einstellung der MSI-Schlüssel für Ihr MAPI-DLL](http://msdn.microsoft.com/en-us/library/ee909494%28VS.85%29.aspx).</span><span class="sxs-lookup"><span data-stu-id="49aa4-121">For more information about registry keys for mail clients that support non-English languages, see [Setting Up the MSI Keys for Your MAPI DLL](http://msdn.microsoft.com/en-us/library/ee909494%28VS.85%29.aspx).</span></span>
