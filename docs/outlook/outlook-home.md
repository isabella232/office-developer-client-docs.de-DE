---
title: Outlook
manager: kelbow
ms.date: 12/13/2017
ms.audience: Developer
localization_priority: Normal
ms.assetid: 59d639e4-d171-4556-a1b9-bf9ecb1f85cf
description: Hier finden Sie Vorgehensweisen, Beispielcode, SDK- und API-Dokumentationen, VBA-Referenzen, Schulungen und technische Artikel zum Entwickeln von Lösungen und zum Anpassen von Outlook.
ms.openlocfilehash: 08dc7cb5ef40f862a09c18375780a26a6f319da5
ms.sourcegitcommit: 9d60cd82b5413446e5bc8ace2cd689f683fb41a7
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 06/11/2018
ms.locfileid: "19795945"
---
# <a name="outlook"></a>Outlook

Hier finden Sie Vorgehensweisen, Beispielcode, SDK- und API-Dokumentationen, VBA-Referenzen, Schulungen und technische Artikel zum Entwickeln von Lösungen und zum Anpassen von Outlook. 

> [!NOTE]
> [!HINWEIS] Haben Sie Interesse an der Entwicklung von Lösungen, mit denen die Funktionen von Office über [mehrere Plattformen](https://docs.microsoft.com/en-us/office/dev/add-ins/overview/office-add-in-availability) erweitert werden können? Schauen Sie sich das neue [Office-Add-Ins-Modell](https://docs.microsoft.com/en-us/office/dev/add-ins/overview/office-add-ins) an. 
  
### <a name="viewed-most"></a>Die am häufigsten aufgerufene
  
- [Outlook VBA-Referenz](https://msdn.microsoft.com/EN-US/library/ee861520.aspx)
  
- [Outlook 2010](https://msdn.microsoft.com/en-us/library/cc313152%28v=office.12%29.aspx)
  
### <a name="office-add-ins"></a>Office-Add-Ins
  
- [Erste Schritte mit Outlook-Add-Ins](https://docs.microsoft.com/en-us/outlook/add-ins/quick-start)
  
- [Plattformübersicht](https://docs.microsoft.com/en-us/office/dev/add-ins/overview/office-add-ins)
  
### <a name="code-samples"></a>Codebeispiele
  
- [Outlook-Codebeispiele für Office Dev Center](https://dev.office.com/code-samples#?filters=outlook)
  
### <a name="other-resources"></a>Weitere Ressourcen
  
- [VBA-Objektbibliothekreferenz für Office](http://msdn.microsoft.com/library/727c4e1c-e13c-7bac-e833-b1322607dfd3%28Office.15%29.aspx)
  
- [VBA-Sprachreferenz für Office](http://msdn.microsoft.com/library/9c1e8386-0309-c52c-856b-963220382eb8%28Office.15%29.aspx)
  
- [Open XML-SDK](http://msdn.microsoft.com/library/f6a9ae68-7989-4208-97f5-3c945137a0ab%28Office.15%29.aspx)
  
- [VSTO-Add-Ins für Office](https://msdn.microsoft.com/en-us/library/jj620922.aspx)
  

