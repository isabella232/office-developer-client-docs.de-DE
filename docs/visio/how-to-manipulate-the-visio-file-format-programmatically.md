...
7. <span data-ttu-id="f8d00-149">Wählen Sie im Kontextmenü für das Anfangs-/Ende-Shape **Dem Zeichenblatt einen Verbinder hinzufügen** aus, und ziehen Sie dann einen Verbinder zwischen dem Anfangs-/Ende- und dem Prozess-Shape auf dem Zeichenbereich, wie in Abbildung 1 dargestellt.</span><span class="sxs-lookup"><span data-stu-id="f8d00-149">On the shortcut menu for the Start/End shape, select **Add One Connector to the Page**, and then draw a connector between the Start/End and Process shapes on the canvas, as shown in Figure 1.</span></span>
...
### <a name="to-create-a-new-solution-in-visual-studio-2012"></a><span data-ttu-id="f8d00-161">Zum Erstellen einer neuen Lösung in Visual Studio 2012</span><span class="sxs-lookup"><span data-stu-id="f8d00-161">To create a new solution in Visual Studio 2012</span></span>
...
2. <span data-ttu-id="f8d00-314">Nach der `RecalcDocument` -Methode in der **Program** -Klasse (oder **Module1** in Visual Basic) aus dem vorherigen Schritt, fügen Sie die folgende Methode hinzu.</span><span class="sxs-lookup"><span data-stu-id="f8d00-314">After the  `RecalcDocument` method in the **Program** class (or **Module1** in Visual Basic) from the previous step, add the following method.</span></span> 
...
3. <span data-ttu-id="f8d00-315">Ersetzen Sie den Code aus dem vorherigen Beispiel im **using**-Block in der **Main**-Methode der **Program**-Klasse (der **Using**-Block der **Main**-Methode in **Module1** in Visual Basic) durch den folgenden Code: </span><span class="sxs-lookup"><span data-stu-id="f8d00-315">Replace the code from the previous example in the **using** block in the **Main** method of the **Program** class (the **Using** block of the **Main** method in **Module1** in Visual Basic), with the following code.</span></span> 
...
4. <span data-ttu-id="f8d00-p140">Drücken Sie F5 zum Debuggen der Lösung. Wenn das Programm die Ausführung abgeschlossen hat, drücken Sie eine beliebige Taste, um es zu verlassen.</span><span class="sxs-lookup"><span data-stu-id="f8d00-p140">Choose the F5 key to debug the solution. When the program has completed running, choose any key to exit.</span></span>
    
5. <span data-ttu-id="f8d00-318">Öffnen Sie die Visio-Package.vsdx-Datei in Visio 2013.</span><span class="sxs-lookup"><span data-stu-id="f8d00-318">Open the Visio Package.vsdx file in Visio 2013.</span></span> 
    
<span data-ttu-id="f8d00-p141">Das Anfangs-/Ende-Shape sollte sich nun 5 cm vom unteren Rand der Zeichnung befinden. Der Verbinder zwischen dem Anfangs-/Ende-Shape und dem Prozess-Shape sollte so umgeleitet worden sein, dass die Änderung im Layout berücksichtigt wird. Wenn die **RecalcDocument**-Eigenschaft nicht zu der Datei hinzugefügt worden wäre, wäre die Shape-Position geändert worden, aber der Verbinder wäre nicht an die neue Position des Shapes umgeleitet worden.</span><span class="sxs-lookup"><span data-stu-id="f8d00-p141">The Start/End shape should now be 2 inches from the bottom edge of the drawing. The connector between the Start/End shape and the Process shape should have rerouted to accommodate the change in layout. If the **RecalcDocument** property had not been added to the file, the shape position would have been changed, but the connector would not have rerouted to the new location of the shape.</span></span> 
  
## <a name="add-a-new-package-part-to-a-package"></a><span data-ttu-id="f8d00-322">Hinzufügen eines neuen Paketteils zu einem Paket</span><span class="sxs-lookup"><span data-stu-id="f8d00-322">Add a new package part to a package</span></span>
<span data-ttu-id="f8d00-323"><a name="vis15_ManipulateFF_AddNewPart"> </a></span><span class="sxs-lookup"><span data-stu-id="f8d00-323"></span></span>

<span data-ttu-id="f8d00-324">Eines der am häufigsten verwendeten Szenarien zum Ändern eines Pakets Datei wird ein neuen Dokumentteils zum Paket hinzufügen.</span><span class="sxs-lookup"><span data-stu-id="f8d00-324">One of the most common scenarios for modifying a file package is adding a new document part to the package.</span></span> <span data-ttu-id="f8d00-325">Wenn Sie die Visio-Zeichnung durch Hinzufügen von Inhalt zum Paket eine Seite hinzufügen möchten, müssen Sie beispielsweise das Paket einer Seite Inhaltsverzeichnis-Webpart hinzufügen.</span><span class="sxs-lookup"><span data-stu-id="f8d00-325">For example, if you want to add a page to the Visio drawing by adding content to the package, you need to add a Page Contents part to the package.</span></span> 
  
<span data-ttu-id="f8d00-326">Das Verfahren zum Hinzufügen eines neuen Teils zu dem Paket ist einfach: </span><span class="sxs-lookup"><span data-stu-id="f8d00-326">The process for adding a new part to the package is straightforward:</span></span> 
  
1. <span data-ttu-id="f8d00-p143">Sie erstellen das XML-Dokument mit den Daten für **PackagePart**. Sie müssen besonders auf die XML-Namespaces Acht geben, die das Schema für den bestimmten Typ des XML-Dokuments festlegen, den Sie erstellen.</span><span class="sxs-lookup"><span data-stu-id="f8d00-p143">You create the XML document with the data for the **PackagePart**. You need to pay special attention to the XML namespaces that govern the schema for the specific type of XML document that you create.</span></span>
    
2. <span data-ttu-id="f8d00-329">Sie erstellen eine neue Datei für die XML-Daten und speichern die Datei an einem Speicherort in **Package**.</span><span class="sxs-lookup"><span data-stu-id="f8d00-329">You create a new file to contain the XML and save the file to a location in the **Package**.</span></span>
    
3. <span data-ttu-id="f8d00-330">Sie erstellen die erforderlichen Beziehungen zwischen dem neuen **PackagePart** und **Package** oder anderen **PackagePart**-Objekten.</span><span class="sxs-lookup"><span data-stu-id="f8d00-330">You create the necessary relationships between the new **PackagePart** and the **Package** or other **PackagePart** objects.</span></span> 
    
4. <span data-ttu-id="f8d00-p144">Sie aktualisieren alle vorhandenen Teile, die auf das neue Teil verweisen müssen. Wenn Sie beispielsweise ein neues Seiteninhaltsteil (eine neue Seite) zu der Datei hinzufügen, müssen Sie auch das Seitenindexteil (/visio/pages/pages.xml-Datei) aktualisieren, um die richtigen Informationen zu der neuen Seite einzuschließen.</span><span class="sxs-lookup"><span data-stu-id="f8d00-p144">You update any existing parts that need to reference the new part. For example, if you add a new Page Contents part (a new page) to the file, you also need to update the Page Index part (/visio/pages/pages.xml file) to include the correct information about the new page.</span></span>
    
<span data-ttu-id="f8d00-333">Verwenden Sie das folgende Verfahren, um eine neue Komponente der Menüband-Erweiterbarkeit in der Visio-Datei zu erstellen.</span><span class="sxs-lookup"><span data-stu-id="f8d00-333">Use the following procedure to create a new Ribbon Extensibility part in the Visio file.</span></span> <span data-ttu-id="f8d00-334">Des neuen Teils der Menüband-Erweiterbarkeit fügt dem Menüband eine neue Registerkarte mit einer Gruppe, die eine einzelne Schaltfläche enthält.</span><span class="sxs-lookup"><span data-stu-id="f8d00-334">The new Ribbon Extensibility part adds to the ribbon a new tab with a single group that contains a single button.</span></span>
  
### <a name="to-create-a-new-package-part"></a><span data-ttu-id="f8d00-335">So erstellen Sie ein neues Paketteil</span><span class="sxs-lookup"><span data-stu-id="f8d00-335">To create a new package part</span></span>

1. <span data-ttu-id="f8d00-336">Nach der `CheckForRecalc` -Methode in der **Program** -Klasse (oder **Module1** in Visual Basic) aus dem vorherigen Verfahren fügen Sie die folgende Methode hinzu.</span><span class="sxs-lookup"><span data-stu-id="f8d00-336">After the  `CheckForRecalc` method in the **Program** class (or **Module1** in Visual Basic) from the previous procedure, add the following method.</span></span> 
...
2. <span data-ttu-id="f8d00-337">Nach der `CreateCustomUI` -Methode in der **Program** -Klasse (oder **Module1** in Visual Basic) aus dem vorherigen Schritt, fügen Sie die folgende Methode hinzu.</span><span class="sxs-lookup"><span data-stu-id="f8d00-337">After the  `CreateCustomUI` method in the **Program** class (or **Module1** in Visual Basic) from the previous step, add the following method.</span></span> 
...
3. <span data-ttu-id="f8d00-338">Ersetzen Sie den gesamten Code im **using**-Block in der **Main**-Methode der **Program**-Klasse (der **Using**-Block der **Main**-Methode in **Module1** in Visual Basic) durch den folgenden Code: </span><span class="sxs-lookup"><span data-stu-id="f8d00-338">Replace all the code in the **using** block in the **Main** method of the **Program** class (the **Using** block of the **Main** method in **Module1** in Visual Basic), with the following code.</span></span> 
...
4. <span data-ttu-id="f8d00-p146">Drücken Sie F5 zum Debuggen der Lösung. Wenn das Programm die Ausführung abgeschlossen hat, drücken Sie eine beliebige Taste, um es zu verlassen.</span><span class="sxs-lookup"><span data-stu-id="f8d00-p146">Choose the F5 key to debug the solution. When the program has completed running, choose any key to exit.</span></span>
    
5. <span data-ttu-id="f8d00-341">Öffnen Sie die Visio-Package.vsdx-Datei in Visio 2013, und wählen Sie dann auf die Registerkarte **Benutzerdefiniert** .</span><span class="sxs-lookup"><span data-stu-id="f8d00-341">Open the Visio Package.vsdx file in Visio 2013, and then choose the **CUSTOM** tab.</span></span> 
    
<span data-ttu-id="f8d00-342">Benutzerdefinierte Menüband sieht wie in Abbildung 2 beim Öffnen der Datei in Visio 2013.</span><span class="sxs-lookup"><span data-stu-id="f8d00-342">The custom ribbon looks like Figure 2 when the file is opened in Visio 2013.</span></span>
  
 <span data-ttu-id="f8d00-343">**Abbildung 2. Benutzerdefinierte Registerkarte im Menüband Visio 2013**</span><span class="sxs-lookup"><span data-stu-id="f8d00-343">**Figure 2. Custom tab in the Visio 2013 ribbon**</span></span>
  
![Eine benutzerdefinierte Registerkarte im Menüband](media/vis15_CustomRibbonTab.PNG)
  
<span data-ttu-id="f8d00-345">Der XML-Code erstellt die `CreateCustomUI` Methode sieht folgendermaßen aus.</span><span class="sxs-lookup"><span data-stu-id="f8d00-345">The XML created by the  `CreateCustomUI` method looks like the following.</span></span> 
...
## <a name="acknowledgements"></a><span data-ttu-id="f8d00-346">Danksagung</span><span class="sxs-lookup"><span data-stu-id="f8d00-346">Acknowledgements</span></span>
<span data-ttu-id="f8d00-347"><a name="vis15_ManipulateFF_Ackn"> </a></span><span class="sxs-lookup"><span data-stu-id="f8d00-347"></span></span>

<span data-ttu-id="f8d00-348">Wir möchten erkennt den Beitrag und die Eingabe von Visio MVP **Al Edlund** bei der Erstellung der Codebeispiele, die in diesem technischen Artikel enthalten sind.</span><span class="sxs-lookup"><span data-stu-id="f8d00-348">We would like to recognize the contribution and input of Visio MVP **Al Edlund** in creating the code examples contained in this technical article.</span></span> <span data-ttu-id="f8d00-349">Al ist eine erkannten Expert in Bearbeiten von der Visio-Dateiformat, einschließlich das Format für Visio-XML-Zeichnung (VDX) und das neue Visio-Dateiformat (.vsdx).</span><span class="sxs-lookup"><span data-stu-id="f8d00-349">Al is a recognized expert in manipulating the Visio file format, including both the Visio XML Drawing format (.vdx) and the new Visio file format (.vsdx).</span></span> <span data-ttu-id="f8d00-350">Al Projekte, die die Visio-Dateiformate programmgesteuert erkunden erstellt wurden, und macht die Strukturen innerhalb.</span><span class="sxs-lookup"><span data-stu-id="f8d00-350">Al has created projects that explore the Visio file formats programmatically and exposes the structures inside.</span></span> 
  
<span data-ttu-id="f8d00-351">Weitere Informationen zur Arbeit mit dem Visio-Dateiformat des Al finden Sie die Links in den folgenden Abschnitt Weitere Ressourcen.</span><span class="sxs-lookup"><span data-stu-id="f8d00-351">For more information about Al's work with the Visio file format, see the links in the Additional Resources section that follows.</span></span>
  
## <a name="see-also"></a><span data-ttu-id="f8d00-352">Siehe auch</span><span class="sxs-lookup"><span data-stu-id="f8d00-352">See also</span></span>
<span data-ttu-id="f8d00-353"><a name="vis15_ManipulateFF_Additional"> </a></span><span class="sxs-lookup"><span data-stu-id="f8d00-353"></span></span>

- <span data-ttu-id="f8d00-354">Von Al Edlund:</span><span class="sxs-lookup"><span data-stu-id="f8d00-354">By Al Edlund:</span></span>
    
  - <span data-ttu-id="f8d00-355">Bei CodePlex [PkgVisio - Visio 2013-XML-Bearbeitung](https://pkgvisio.codeplex.com/documentation) -Projekt.</span><span class="sxs-lookup"><span data-stu-id="f8d00-355">[pkgVisio - Visio 2013 XML manipulation](https://pkgvisio.codeplex.com/documentation) project on CodePlex.</span></span> 
    
  - <span data-ttu-id="f8d00-356">Sie Videos auf YouTube die aus [pkgVisio_pt1](https://www.youtube.com/watch?v=7LvDKJuP9oQ&amp;feature=youtu.be) .</span><span class="sxs-lookup"><span data-stu-id="f8d00-356">[pkgVisio_pt1 ](https://www.youtube.com/watch?v=7LvDKJuP9oQ&amp;feature=youtu.be) video on YouTube.</span></span> 
    
  - <span data-ttu-id="f8d00-357">Sie Videos auf YouTube die aus [pkgVisio_pt2](https://www.youtube.com/watch?v=ZIWSXhNSkG8&amp;feature=youtu.be) .</span><span class="sxs-lookup"><span data-stu-id="f8d00-357">[pkgVisio_pt2 ](https://www.youtube.com/watch?v=ZIWSXhNSkG8&amp;feature=youtu.be) video on YouTube.</span></span> 
    
- [<span data-ttu-id="f8d00-358">Visio Developer Center</span><span class="sxs-lookup"><span data-stu-id="f8d00-358">Visio Developer Center</span></span>](https://msdn.microsoft.com/office/aa905478.aspx)
    
- [<span data-ttu-id="f8d00-359">Bearbeiten von Dokumenten im Office Open XML-Formate</span><span class="sxs-lookup"><span data-stu-id="f8d00-359">Manipulate Office Open XML Formats Documents</span></span>](https://msdn.microsoft.com/library/aa982683%28v=office.12%29.aspx)
    
- [<span data-ttu-id="f8d00-360">Erstellen eines Dokuments mit Namespaces (c#) (LINQ to XML)</span><span class="sxs-lookup"><span data-stu-id="f8d00-360">Create a Document with Namespaces (C#) (LINQ to XML)</span></span>](https://msdn.microsoft.com/library/bb387075.aspx)
    
- [<span data-ttu-id="f8d00-361">Hinzufügen von benutzerdefinierten XML-Komponenten für Dokumente ohne Starten von Microsoft Office</span><span class="sxs-lookup"><span data-stu-id="f8d00-361">Add Custom XML Parts to Documents Without Starting Microsoft Office</span></span>](https://msdn.microsoft.com/library/bb608597%28VS.90%29.aspx)
